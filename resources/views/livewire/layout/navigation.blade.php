...
<nav x-data="{ open: false }" class="expense-nav">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center rounded-lg border border-[#5b4b3f] px-3 py-2 text-sm font-medium leading-4 text-[#f7eadb] transition hover:border-[#d8b99a] hover:bg-white/10 hover:text-white focus:outline-none">
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center rounded-lg border border-[#5b4b3f] p-2 text-[#f7eadb] transition hover:bg-white/10 hover:text-white focus:outline-none">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="expense-mobile-menu hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-[#332b23]">
            <div class="px-4">
                <div class="font-medium text-base text-[#fff8ef]" x-data="{{ json_encode(['name' => auth()->user()->name]) }}" x-text="name" x-on:profile-updated.window="name = $event.detail.name"></div>
                <div class="font-medium text-sm text-[#d8b99a]">{{ auth()->user()->email }}</div>
            </div>
